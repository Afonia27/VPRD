...
long tp1; // Time for pump
long tp2; // Time for pump final
int error = 5; // used for checking connection with the robot
int counter = 0;
int counter2 = 1;
int k = 18 ; //Variable used for recieved data byte 0
...
byte latest_data[3]; // Array for the latest data (Used in Clear Channel algorythm) 
...
void loop() {
  ...
  delay(5);// To recieve the data module needs to start listening
...
//------------------------------------------ UNCONNECTED MODE LISTEN ---------------------------------
  ...
  if (battery < 15){
  lcd.setCursor(4,2); // Print the infromation on the screen
  lcd.print("R Battery is Low"); 
 }
  
else if (battery > 15){
  lcd.setCursor(0,2); // Print the infromation on the screen
  lcd.print("                    "); 
 }
  lcd.setCursor(4,0); // Print the infromation on the screen
  lcd.print("Main Driving"); 
  lcd.setCursor(4,1);
  lcd.print("Drive Safely!");
  lcd.setCursor(0,3);
  lcd.print("Ch:");lcd.print(Cchannel);lcd.print("     ");lcd.print("Rbat:");lcd.print(battery);lcd.print("%");lcd.print("  ");
  ...
  if (pf == 1){
  tp2=(((millis()-tp1)-100)/1000);
  lcd.setCursor(14,1);lcd.print("T:");lcd.print(tp2);
  }
  if (pf == 0){
...
if (k != 18){ // Initiate Clean Channel algorythm if checksum dont match 
  flag1 = 1;
}
